...
def checkNoNans(matrix_list):
    ...
    for i, M in enumerate(matrix_list):
        if np.any(np.isnan(np.array(M))) or np.any(np.isinf(np.array(M))):
            raise Exception(
                'Matrix index %i in list has a NaN or infinite element' % i)
...
def invertFast(A, d):
    ...
    M = np.diag(d_inv) - inv_d_squared * np.dot(np.dot(A,
                                                       np.linalg.inv(np.eye(k, k) + np.dot(A.T, mult_diag(d_inv, A)))), A.T)
    return M
...
def Estep(Y, A, mus, sigmas, decay_coef):
    ...
    for i in range(N):
        ...
        zero_indices = np.array(
            [True for a in range(K)] + [np.abs(Y_i[j]) < 1e-6 for j in range(D)])
        dim = K + D  # this is dimension of matrix

        # 2. compute P(Z, X_0 | Y_+)
        mu_c, sigma_c, augmentedA_0, augmentedA_plus, augmented_D, sigma_22_inv = calcConditionalDistribution(
            A, mus, sigmas, np.array([np.abs(Y_i[j]) < 1e-6 for j in range(D)]), Y_i[~Y_is_zero])

        # 3. compute P(Z, X_0 | Y_+, Y_0)
        dim = len(sigma_c)
        matrixToInvert = computeMatrixInLastStep(
            A, np.abs(Y[i, :]) < 1e-6, sigmas, K, sigma_c, decay_coef, sigma_22_inv)
        if (Y_is_zero).sum() < D:
            magical_matrix = 2 * decay_coef * (mult_diag(augmented_D, matrixToInvert) + augmentedA_0 * (
                np.eye(K) - augmentedA_plus.T * sigma_22_inv * augmentedA_plus) * (augmentedA_0.T * matrixToInvert))
        else:
            magical_matrix = 2 * decay_coef * \
                (mult_diag(augmented_D, matrixToInvert) +
                 augmentedA_0 * (augmentedA_0.T * matrixToInvert))
        magical_matrix[:, :K] = 0
        ...
        if (Y_is_zero).sum() < D:
            sigma_xz = np.array(sigma_c - mult_diag(augmented_D, np.array(magical_matrix), left=False) - (
                magical_matrix * augmentedA_0) * ((np.eye(K) - augmentedA_plus.T * sigma_22_inv * augmentedA_plus) * augmentedA_0.T))
        else:
            sigma_xz = np.array(sigma_c - mult_diag(augmented_D, np.array(
                magical_matrix), left=False) - (magical_matrix * augmentedA_0) * augmentedA_0.T)
        mu_xz = np.array(
            np.matrix(np.eye(dim) - magical_matrix) * np.matrix(mu_c))
        EZ[i, :] = mu_xz[:K, 0]
        EX[i, Y_is_zero] = mu_xz[K:, 0]
        EX2[i, Y_is_zero] = mu_xz[K:, 0] ** 2 + np.diag(sigma_xz[K:, K:])
        EZZT[i, :, :] = np.dot(np.atleast_2d(mu_xz[:K, :]), np.atleast_2d(
            mu_xz[:K, :].transpose())) + sigma_xz[:K, :K]
        EXZ[i, Y_is_zero, :] = np.dot(
            mu_xz[K:], mu_xz[:K].transpose()) + sigma_xz[K:, :K]
...
def computeMatrixInLastStep(A, zero_indices, sigmas, K, sigma_c, decay_coef, sigma_22_inv):
    ...
    E_xx = sigma_c[K:, :][:, K:]
    E_xz = sigma_c[K:, :][:, :K]
    E_00_prime_inv = np.matrix(
        invertFast(A_0, sigmas_0 ** 2 + 1 / (2. * decay_coef)))
    ...
    if (E_plusplus_inv.shape[0] == 0) or (E_plusplus_inv.shape[1] == 0):
        ...
        b_inv = np.linalg.inv(
            (np.matrix(A_0).T * E_00_prime_inv) * np.matrix(A_0))
        innermost_inverse = applyWoodburyIdentity(
            -E_plusplus_inv, b_inv, np.matrix(A_plus))
        inv_matrix = (1 / (2. * decay_coef)) * (E_00_prime_inv - (E_00_prime_inv * A_0)
                                                * (A_plus.T * innermost_inverse * A_plus) * (A_0.T * E_00_prime_inv))
    dim = len(sigma_c)
    M = np.zeros([dim, dim])
    M[:K, :K] = np.eye(K)
...
def Mstep(Y, EZ, EZZT, EX, EXZ, EX2, oldA, old_mus, old_sigmas, old_decay_coef, singleSigma=False):
    ...
    c[:K, :] = (tiled_Y_is_zero * EXZ + (1 - tiled_Y_is_zero)
                * tiled_Y * tiled_EZ).sum(axis=0).transpose()
    ...
    A_product = np.tile(np.reshape(
        A, [1, D, K]), [K, 1, 1]) * (np.tile(np.reshape(A, [1, D, K]), [K, 1, 1]).T)
    for i in range(N):
        ...
        EM2[i, :] = ezzt_sum + 2 * test_sum[i, :] * \
            tiled_mus[i, :] + tiled_mus[i, :] ** 2
    sigmas = (Y_is_zero * (EX2 - 2 * EXM + EM2) + (1 - Y_is_zero)
              * (Y ** 2 - 2 * Y * EM + EM2)).sum(axis=0)
    sigmas = np.atleast_2d(np.sqrt(sigmas / N)).transpose()
    if singleSigma:
        sigmas = np.mean(sigmas) * np.ones(sigmas.shape)
    decay_coef = minimize(lambda x: decayCoefObjectiveFn(
        x, Y, EX2), old_decay_coef, jac=True, bounds=[[1e-8, np.inf]])
...
def calcConditionalDistribution(A, mus, sigmas, zero_indices, observed_values):
    ...
    augmented_zero_indices = np.array(
        [True for a in range(K)] + list(zero_indices))
    ...
    sigma_11[K:, K:] = sigma_11[K:, K:] + \
        np.diag(sigmas[zero_indices][:, 0] ** 2)
    augmented_D = np.array(
        [0 for i in range(K)] + list(sigmas[zero_indices][:, 0] ** 2))

    if len(observed_values) == 0:
        sigma_x = augmentedA * augmentedA.T
        sigma_x[K:, K:] = sigma_x[K:, K:] + \
            np.diag(sigmas[zero_indices][:, 0] ** 2)
        return mu_x, sigma_x, augmentedA_0, augmentedA_plus, augmented_D, np.array([[]])
    sigma_22_inv = np.matrix(
        invertFast(A[~zero_indices, :], sigmas[~zero_indices] ** 2))
    mu_0 = mu_x[augmented_zero_indices, :] + augmentedA_0 * \
        (augmentedA_plus.T * (sigma_22_inv * mu_diff))
    sigma_0 = sigma_11 - augmentedA_0 * \
        (augmentedA_plus.T * sigma_22_inv * augmentedA_plus) * augmentedA_0.T
    assert((mu_0.shape[0] == zero_indices.sum() + K) and (mu_0.shape[1] == 1))
    assert(sigma_0.shape[0] == zero_indices.sum() +
           K and sigma_0.shape[1] == zero_indices.sum() + K)
...
def decayCoefObjectiveFn(x, Y, EX2):
    ...
    with warnings.catch_warnings():
        ...
        grad = sum(
            sum(Y_is_zero * (-EX2) + (1 - Y_is_zero) * y_squared * exp_ratio))
...
def combineMatrices(y_indices, all_EZs, all_EZZTs, all_EXs, all_EXZs, all_EX2s):
    ...
    assert(len(all_EZs) == len(all_EZZTs) == len(all_EXs) == len(
        all_EXZs) == len(all_EX2s) == len(y_indices))
    for block in range(n_blocks):
...
def testInputData(Y):
    ...
    if column_is_all_zero.sum() > 0:
        print("Warning: Your Y matrix has %i columns which are entirely zero; filtering these out before continuing." % (
...
def runEMAlgorithm(Y, K, singleSigma=False, n_blocks=None):
    ...
    print('Running block zero-inflated factor analysis with N = %i, D = %i, K = %i, n_blocks = %i' %
          (N, D, K, n_blocks))
    # generate blocks.
    ...
    np.random.seed(23)
    A, mus, sigmas, decay_coef = initializeParams(
        Y, K, singleSigma=singleSigma)
    ...
    while n_iter < max_iter:
        for i in range(N):
            ...
            for block in range(n_blocks):
                ...
                block_EZ, block_EZZT, block_EX, block_EXZ, block_EX2 = Estep(
                    np.array([Y_to_use]), A_to_use, mus_to_use, sigmas_to_use, decay_coef)
            ...
            EZ[i], EZZT[i], EX[i], EXZ[i], EX2[i] = combineMatrices(
                y_indices_to_use[i], block_EZs, block_EZZTs, block_EXs, block_EXZs, block_EX2s)
        new_A, new_mus, new_sigmas, new_decay_coef = Mstep(
            Y, EZ, EZZT, EX, EXZ, EX2, A, mus, sigmas, decay_coef, singleSigma=singleSigma)
        try:
            checkNoNans(
                [EZ, EZZT, EX, EXZ, EX2, new_A, new_mus, new_sigmas, new_decay_coef])
        except:
            print(
        ...
        for new, old in [[new_mus, mus], [new_A, A], [new_sigmas, sigmas], [new_decay_coef, decay_coef]]:
            rel_param_change = np.mean(
                np.abs(new - old)) / np.mean(np.abs(new))
        ...
        if paramsNotChanging:
            print('Param change below threshold %2.3e after %i iterations' %
                  (param_change_thresh, n_iter))
            break
        if n_iter >= max_iter:
            print('Maximum number of iterations reached; terminating loop')
        n_iter += 1
    params = {'A': A, 'mus': mus, 'sigmas': sigmas,
              'decay_coef': decay_coef, 'X': EX}
    return EZ, params
...
def fitModel(Y, K, singleSigma=False, n_blocks=None, p0_thresh=.95):
    ...
    print('Filtering out all genes which are zero in more than %2.1f%% of samples. To change this, change p0_thresh.' % (
        p0_thresh * 100))
    ...
    while results is None:
        ...
        print('Previously, maximum fraction of zeros for a gene was %2.3f; now lowering that to %2.3f and rerunning ZIFA' % (
            max_zero_frac, new_max_zero_frac))
        Y = Y[:, Y_is_zero.mean(axis=0) < new_max_zero_frac]
        print(
            'After filtering out genes with too many zeros, %i samples and %i genes' % Y.shape)
        results = runEMAlgorithm(
            Y, K, singleSigma=singleSigma, n_blocks=n_blocks)
    return results
